...
public class ManagementBuddyMemoryAllocationImpl extends ManagementAbstract implements ManagementBuddyMemoryAllocation {
	...
	private static ManagementBuddyMemoryAllocation instance;
    ...
    public static ManagementBuddyMemoryAllocation getInstance() {
		if (instance == null) {
			instance = new ManagementBuddyMemoryAllocationImpl();
		}
		return instance;
	}
	
	private NodeBuddy root;
	private HashMap<String, Color> mapColors;
	private List<Color> listColor;
	private List<NodeProcess> listRunningProcesses;
	...
	protected void initialize() {
		root = null;
		listRunningProcesses = new ArrayList<NodeProcess>();
	...
	private NodeProcess getSpaceNode(String name) {
		try {
			if (name == null) {
				throw new NullPointerException();
			}
			NodeProcess result = null;
			for (NodeProcess space : listRunningProcesses) {
	...
	private NodeBuddy getMinBuddyNode(List<NodeBuddy> list) {
		try {
			if (list == null) {
				throw new NullPointerException();
			}
			NodeBuddy minBuddyNode = null;
			Integer minValue = null;
			for (NodeBuddy node : list) {
		...
		try {
			if ((processName == null) || (processName == "") || (processName.length() == 0)) {
				...
				if (getSpaceNode(processName) == null) {
					List<NodeBuddy> list = root.findPossibleBuddyNodes(processSize);				
					NodeBuddy buddy = getMinBuddyNode(list);
					if (buddy != null) {
						NodeProcess node = buddy.insertSpace(processName, processSize);
						if (node != null) {
	...
	private void stopProcess() {
		try {
			...
			NodeProcess space = getSpaceNode(processName);
			if (space != null) {
				NodeBuddy parent = space.getParent();
				parent.removeSpace();
	...
	public Integer getProcessSpace() {
		Integer sum = 0;
		if (listRunningProcesses != null) {
			for (NodeProcess process : listRunningProcesses) {
	...
	public Integer getRestSpace() {
		...
		if (listRunningProcesses != null) {
			for (NodeProcess process : listRunningProcesses) {
				NodeBuddy parent = process.getParent();
	...
	public List<NodeProcess> getListRunningProcesses() {
		List<NodeProcess> list = new ArrayList<NodeProcess>(listRunningProcesses);
		return list;
	}
