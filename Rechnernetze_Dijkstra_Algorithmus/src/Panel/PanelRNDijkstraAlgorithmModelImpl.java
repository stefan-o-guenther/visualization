...
import panel.PanelDrawingAbstract;
...
public class PanelRNDijkstraAlgorithmModelImpl extends PanelDrawingAbstract {
	...
	private void drawEdgeLine(OutputEdge edge, Integer x1, Integer y1, Integer x2, Integer y2) {
	...
	private void drawEdgeText(OutputEdge edge, Integer x, Integer y) {
		EnumSurface surface = dijkstra.getSurface();
	...
	private void drawEdgeUV() {
		...
		OutputEdge edge = dijkstra.getEdge(nameU, nameV);
		if (edge != null) {
	...
	private void drawEdgeUW() {
		...
		EnumSurface surface = dijkstra.getSurface();
		OutputEdge edge = dijkstra.getEdge(nameU, nameW);
		if (edge != null) {
	...
	private void drawEdgeUX() {
		...
		OutputEdge edge = dijkstra.getEdge(nameU, nameX);
		if (edge != null) {
			drawEdgeText(edge, x1+difPicX+(difX/2)-6, y1+difPicY+(difY/2)-6);
	...
	private void drawEdgeVW() {
		String nameV = this.listNodeNames.get(1);
		String nameW = this.listNodeNames.get(2);
		OutputEdge edge = dijkstra.getEdge(nameV, nameW);
		if (edge != null) {
	...
	private void drawEdgeVX() {
		String nameV = this.listNodeNames.get(1);
		String nameX = this.listNodeNames.get(3);
		OutputEdge edge = dijkstra.getEdge(nameV, nameX);
		if (edge != null) {
	...
	private void drawEdgeWX() {
		String nameW = this.listNodeNames.get(2);
		String nameX = this.listNodeNames.get(3);
		OutputEdge edge = dijkstra.getEdge(nameW, nameX);
		if (edge != null) {
	...
	private void drawEdgeWY() {
		...
		OutputEdge edge = dijkstra.getEdge(nameW, nameY);
		if (edge != null) {
	...
	private void drawEdgeWZ() {
		...
		OutputEdge edge = dijkstra.getEdge(nameW, nameZ);
		if (edge != null) {
	...
	private void drawEdgeXY() {
		String nameX = this.listNodeNames.get(3);
		String nameY = this.listNodeNames.get(4);
		OutputEdge edge = dijkstra.getEdge(nameX, nameY);
		if (edge != null) {
	...
	private void drawEdgeYZ() {		
		String nameY = this.listNodeNames.get(4);
		String nameZ = this.listNodeNames.get(5);
		OutputEdge edge = dijkstra.getEdge(nameY, nameZ);
		if (edge != null) {
			drawEdgeText(edge, x2+difPicX-(difX/2)-6, y1+difPicY+(difY/2)-6);
