...
import packetarq.PacketArqAckImpl;
import packetarq.PacketArqData;
import packetarq.PacketArqDataImpl;
import packetarq.PacketArqNakImpl;
import packetarq.PacketArqResult;
...
import actor.ActorReceiverImpl;
import actor.ActorSender;
import actor.ActorSenderImpl;
...
public abstract class ARQProtocolStrategyAbstract implements ARQProtocolStrategy {
	...
	private void initializeLists() {
		...
		this.arraySender = new ActorSender[length.intValue()];
		this.arrayReceiver= new ActorReceiver[length.intValue()];
		int l = this.length.intValue();		
		for (int i = 0; i < l; i++) {
			arraySender[i] = new ActorSenderImpl();
			arrayReceiver[i] = new ActorReceiverImpl();
		}
	}
	...
	private ActorSender[] arraySender;
	private ActorReceiver[] arrayReceiver;
	...
	protected ActorSender getSender(Integer number) {
	...
	protected Boolean isAnySent() {
		...
		for (int i = base; i <= max; i++) {
			ActorSender sender = this.arraySender[i];
	...
	protected ActorReceiver getReceiver(Integer number) {
		Checker.checkIfIntegerNotLessZero(number);
	...
	private void executeReceiver() {		
		...
		while (i < this.getSizeListPacket().intValue()) {
			...
			if ((packetType == EnumPacketType.DATA) && (position > yMax)) {
				this.removerPacket(packet);
				ActorReceiver receiver = this.arrayReceiver[packetNumber];
	...
	private void executeFinish() {
		...
		int sizeSender = 0;
		for (ActorSender sender : this.arraySender) {
